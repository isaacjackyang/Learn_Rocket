...
def write_generation_report(
    ...
    lines = [
        ...
        f"- Hard reset mode: `{plan.hard_reset_mode}`",
        f"- Mutation scale: `{plan.mutation_scale}`",
        f"- Random injection ratio: `{plan.random_injection_ratio}`",
        *( [f"- Blocked region stage: `{plan.blocked_region_stage_id}` ({len(plan.blocked_regions)} regions)"] if plan.blocked_regions else [] ),
        *( [f"- Effective population size: `{plan.effective_population_size}`"] if plan.effective_population_size else [] ),
        *( [f"- Effective seeds per experiment: `{plan.effective_seeds_per_experiment}`"] if plan.effective_seeds_per_experiment else [] ),
        *( [f"- Reentry boost remaining: `{plan.reentry_boost_remaining}` generation(s)"] if plan.reentry_boost_remaining else [] ),
