...
class BalloonPoppingEnv(gym.Env):
    ...
    def __generate_balloon_flights(self):
        ...
        expected_balloons = release_steps.shape[0]
        if num_balloons != expected_balloons:
            if num_balloons > expected_balloons:
                self._balloon_flights = self._balloon_flights[:expected_balloons, :, :]
            else:
                pad_count = expected_balloons - num_balloons
                padding = np.repeat(self._balloon_flights[-1:, :, :], pad_count, axis=0)
                self._balloon_flights = np.concatenate([self._balloon_flights, padding], axis=0)
            num_balloons = expected_balloons
        ...
        # Gather shifted trajectories along the time axis.
        # `take_along_axis` is compatible with recent NumPy versions where the
        # previous advanced-indexing broadcast pattern can raise shape errors.
        shifted = np.take_along_axis(
            self._balloon_flights,
            source_idx[:, np.newaxis, :],
            axis=2,
        )
